...
function appointmentDisplayRange(start, end) {
    ...
    if (Number.isNaN(startDate.getTime())) return 'Creneau selectionne';
...
function openQuickAppointmentModal(start, end = null) {
    ...
    endInput.value = end ? appointmentIso(endDate) : '';
...
function initializeAppointmentCalendar() {
    ...
    if (!root || !calendarElement || appointmentCalendar) return;
    if (!window.FullCalendar) {
        window.setTimeout(initializeAppointmentCalendar, 150);
        return;
    }
