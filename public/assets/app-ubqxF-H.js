...
function syncExpenseSummary(payload, sourceUrl) {
    const stats = document.getElementById('expenseStats');
    if (stats && payload.data?.statsHtml) {
        stats.innerHTML = payload.data.statsHtml;
    }

    const exportLink = document.querySelector('[data-expense-export]');
    if (!exportLink) return;

    const exportUrl = new URL(exportLink.dataset.expenseExportBase || exportLink.href, window.location.origin);
    ['q', 'category', 'status', 'paymentMethod', 'active', 'dateFrom', 'dateTo', 'minAmount', 'maxAmount'].forEach((key) => {
...
async function loadExpenseGrid(search, page = 1) {
    ...
    syncExpenseSummary(payload, url);
}
...
function fishReceptionDateTimeToMinutes(dateValue, timeValue) {
    const time = fishPackagingTimeToMinutes(timeValue);
    ...
    return Math.floor(timestamp / 60000) + time;
}
...
function fishFreezingInputValue(value) {
    const rounded = Math.max(0, Math.round(Number(value || 0) * 1000) / 1000);
    const formatted = rounded.toFixed(3).replace(/\.?0+$/, '');

    return formatted === '' ? '0' : formatted;
}

function autoAdjustFishFreezingFinished(form) {
    const quantityField = form.querySelector('[data-fish-freezing-finished]');
    ...
    const waste = fishReceptionValue(form, 'poidsDechetsTraitement');
    const loss = fishReceptionValue(form, 'poidsPertesTraitement');
    const adjusted = Math.max(0, source - currentFrozen - waste - loss);

    if (Math.abs(parseDecimalInput(quantityField.value) - adjusted) > 0.0005) {
        quantityField.value = fishFreezingInputValue(adjusted);
        quantityField.dispatchEvent(new Event('input', {bubbles: true}));
    }
}

function syncFishFreezingForm(form, options = {}) {
    if (options.autoAdjustFinished) {
        autoAdjustFishFreezingFinished(form);
    }
...
function initializeFishFreezingForms(root = document) {
    root.querySelectorAll('[data-fish-freezing-form]').forEach((form) => {
        syncFishFreezingForm(form, {autoAdjustFinished: true});
        ...
        form.querySelectorAll('[data-fish-freezing-balance-field]').forEach((field) => {
            const listener = () => syncFishFreezingForm(form, {
                autoAdjustFinished: field.matches('[data-fish-freezing-waste], [data-fish-freezing-loss]'),
            });
...
function syncFishPackagingQuantityAvailability(form) {
    ...
    if (exceeded) {
        ...
        if (feedback) {
            feedback.textContent = `Quantite insuffisante : ${coutFormatCompact(requested)} kg a emballer, ${coutFormatCompact(available)} kg cristallises disponibles.`;
...
function fishTunnelExitDurationHours(form) {
    const summary = form.querySelector('[data-fish-tunnel-exit-summary]');
    const entryDate = fishReceptionField(form, 'dateEntreeTunnel')?.value || summary?.dataset.entryDate;
    const entryTime = fishReceptionField(form, 'heureEntreeTunnel')?.value || summary?.dataset.entryTime;
    const exitDate = fishReceptionField(form, 'dateSortieTunnel')?.value || entryDate;
    const start = fishReceptionDateTimeToMinutes(entryDate, entryTime);
    let end = fishReceptionDateTimeToMinutes(exitDate, fishReceptionField(form, 'heureSortieTunnel')?.value);
    if (start === null || end === null) return 0;
    if (end < start) end += 1440;
...
function syncFishTunnelExitForm(form) {
    const summary = form.querySelector('[data-fish-tunnel-exit-summary]');
    const hasEntryTime = Boolean(fishReceptionField(form, 'heureEntreeTunnel')?.value || summary?.dataset.entryTime);
    const duration = fishTunnelExitDurationHours(form);
    const output = form.querySelector('[data-fish-tunnel-exit-output="duration"]');
    const status = form.querySelector('[data-fish-tunnel-exit-status]');
    ...
    if (!hasEntryTime) {
        status.classList.add('alert-danger');
        status.textContent = "Heure d'entrée tunnel manquante. Renseignez l'étape congélation avant d'entrer en stockage.";
    } else if (duration <= 0) {
        status.classList.add('alert-secondary');
        status.textContent = "Renseignez l'heure de sortie tunnel pour calculer la durée utilisée dans les charges horaires.";
    } else if (duration <= 3) {
        status.classList.add('alert-success');
        status.textContent = `Sortie tunnel cohérente : ${coutFormat(duration)} h de tunnel. Cette durée sera retenue dans l'estimation des charges horaires.`;
    } else if (duration <= 4) {
        status.classList.add('alert-warning');
...
function initializeFishTunnelExitForms(root = document) {
    root.querySelectorAll('[data-fish-tunnel-exit-form]').forEach((form) => {
        syncFishTunnelExitForm(form);
        if (form.dataset.fishTunnelExitInitialized === 'true') return;

        form.dataset.fishTunnelExitInitialized = 'true';
        form.addEventListener('input', (event) => {
            if (event.target.matches('input, textarea')) {
                syncFishTunnelExitForm(form);
            }
...
const interimPassportOcrScriptUrl = 'https://cdn.jsdelivr.net/npm/tesseract.js@5.1.1/dist/tesseract.min.js';
let interimPassportOcrPromise = null;

function interimPassportField(form, name) {
    return form.querySelector(`[name$="[${name}]"]`);
}

function setInterimPassportStatus(scanner, message, tone = 'secondary') {
    const status = scanner?.querySelector('[data-interim-passport-status]');
    if (!status) return;
...
function setInterimPassportWarning(scanner, message = '') {
    const warning = scanner?.querySelector('[data-interim-passport-warning]');
    if (!warning) return;
...
function setInterimPassportProgress(scanner, progress = 0) {
    const wrap = scanner?.querySelector('[data-interim-passport-progress-wrap]');
    const bar = scanner?.querySelector('[data-interim-passport-progress]');
    if (!wrap || !bar) return;
...
function loadInterimPassportOcr() {
    if (window.Tesseract?.recognize) {
        return Promise.resolve(window.Tesseract);
    }

    if (interimPassportOcrPromise) {
        return interimPassportOcrPromise;
    }

    interimPassportOcrPromise = new Promise((resolve, reject) => {
        const script = document.createElement('script');
        script.src = interimPassportOcrScriptUrl;
        script.async = true;
        script.onload = () => {
            if (window.Tesseract?.recognize) {
            ...
            reject(new Error('Moteur OCR indisponible.'));
        };
...
function loadInterimPassportImage(file) {
    return new Promise((resolve, reject) => {
        ...
        image.onerror = () => {
            URL.revokeObjectURL(url);
            reject(new Error('Image passeport illisible.'));
        };
        image.src = url;
    });
}

async function createInterimPassportMrzImage(file) {
    const image = await loadInterimPassportImage(file);
    const sourceWidth = image.naturalWidth || image.width;
    const sourceHeight = image.naturalHeight || image.height;
    const cropY = Math.floor(sourceHeight * 0.58);
    const cropHeight = Math.max(1, sourceHeight - cropY);
    const scale = Math.min(3, Math.max(1.6, 2800 / Math.max(1, sourceWidth)));
    const canvas = document.createElement('canvas');
    canvas.width = Math.round(sourceWidth * scale);
    canvas.height = Math.round(cropHeight * scale);
    const context = canvas.getContext('2d', {willReadFrequently: true});
    if (!context) {
        throw new Error('Previsualisation passeport impossible.');
    }
    ...
    return new Promise((resolve, reject) => {
        canvas.toBlob((blob) => {
            ...
            reject(new Error('Image passeport invalide.'));
        }, 'image/png');
    });
}

function normalizeInterimMrzLine(line) {
    return String(line || '')
        .toUpperCase()
        .replace(/[«‹＜]/g, '<')
        .replace(/\s+/g, '')
...
function cleanInterimMrzDate(value) {
    return String(value || '')
        .toUpperCase()
        .replace(/[OQD]/g, '0')
        .replace(/[IL]/g, '1')
...
function parseInterimMrzDate(value, type = 'birth') {
    const date = cleanInterimMrzDate(value).slice(0, 6);
...
function interimCountryName(code) {
    const countries = {
        CIV: "Cote d'Ivoire",
        MAR: 'Maroc',
        FRA: 'France',
...
function extractInterimPassportMrz(text) {
    const lines = String(text || '')
        .split(/\r?\n/)
        .map(normalizeInterimMrzLine)
    ...
    for (let index = 0; index < lines.length - 1; index += 1) {
        ...
        if (start > 0) line1 = line1.slice(start);
        if (!line1.startsWith('P<') || line2.length < 30) continue;
        ...
        const issueCountry = line1.slice(2, 5).replace(/</g, '');
        const nationalityCode = line2.slice(10, 13).replace(/</g, '');
        const passportNumber = line2.slice(0, 9).replace(/</g, '').replace(/[^A-Z0-9]/g, '');
        const birthDate = parseInterimMrzDate(line2.slice(13, 19), 'birth');
        const passportExpiresAt = parseInterimMrzDate(line2.slice(21, 27), 'expiry');
        const gender = ['M', 'F', 'X'].includes(line2.slice(20, 21)) ? line2.slice(20, 21) : '';

        if (!passportNumber || !birthDate) continue;

        return {
            lastName,
            firstName,
            passportNumber,
            passportIssueCountry: issueCountry,
            nationality: interimCountryName(nationalityCode),
            gender,
            birthDate,
            passportExpiresAt,
            passportMrz: `${line1}\n${line2}`,
...
function normalizeInterimOcrText(text) {
    return String(text || '')
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '')
        .toUpperCase();
...
function parseInterimVisualDate(day, month, year) {
    const d = Number(day);
    ...
    if (m < 1 || m > 12 || d < 1 || d > 31 || parsed.getUTCFullYear() !== y || parsed.getUTCMonth() !== m - 1 || parsed.getUTCDate() !== d) {
        return '';
    }
...
function extractInterimPassportVisualDetails(text, mrzData) {
    const details = {};
    const normalized = normalizeInterimOcrText(text);
    const dateMatches = [...normalized.matchAll(/(?:^|\D)(\d{2})[\s./-]+(\d{2})[\s./-]+(\d{2,4})(?=\D|$)/g)]
        .map((match) => parseInterimVisualDate(match[1], match[2], match[3]))
        .filter(Boolean);
    const uniqueDates = [...new Set(dateMatches)];
    const today = new Date().toISOString().slice(0, 10);
    const issueCandidates = uniqueDates.filter((date) => date !== mrzData.birthDate && date !== mrzData.passportExpiresAt && date <= today);
    if (issueCandidates.length > 0) {
        details.passportIssuedAt = issueCandidates.sort().at(-1);
    }

    const lines = normalized.split(/\r?\n/).map((line) => line.trim()).filter(Boolean);
    for (let index = 0; index < lines.length; index += 1) {
        if (!/(LIEU.*NAISSANCE|PLACE.*BIRTH)/.test(lines[index])) continue;
        const candidate = (lines[index + 1] || '')
            .replace(/[^A-Z -]/g, '')
            .replace(/\s+/g, ' ')
            .trim();
        if (candidate.length >= 3 && !/(DATE|AUTHORITY|SIGNATURE)/.test(candidate)) {
            details.birthPlace = candidate;
            break;
        }
    }
...
function setInterimPassportFormValue(form, name, value) {
    if (value === undefined || value === null || value === '') return false;

    const input = interimPassportField(form, name);
...
function applyInterimPassportData(form, data) {
    let count = 0;
    Object.entries({
        ...
        if (setInterimPassportFormValue(form, name, value)) count += 1;
    });
...
async function recognizeInterimPassportImage(Tesseract, image, scanner, progressOffset = 0, progressScale = 1) {
    const result = await Tesseract.recognize(image, 'eng', {
        logger: (event) => {
            if (event.status === 'recognizing text') {
                setInterimPassportProgress(scanner, progressOffset + ((event.progress || 0) * progressScale));
            }
        },
    });
...
async function handleInterimPassportScan(input) {
    const form = input.closest('[data-interim-worker-form]');
    const scanner = input.closest('[data-interim-passport-scanner]');
    const button = scanner?.querySelector('[data-interim-passport-button]');
    const file = input.files?.[0];
    if (!form || !scanner || !file) return;

    button?.setAttribute('disabled', 'disabled');
    setInterimPassportWarning(scanner, '');
    setInterimPassportStatus(scanner, 'Chargement OCR...', 'primary');
    setInterimPassportProgress(scanner, 0.05);

    try {
        const Tesseract = await loadInterimPassportOcr();
        setInterimPassportStatus(scanner, 'Lecture zone MRZ...', 'primary');
        const mrzImage = await createInterimPassportMrzImage(file);
        const mrzText = await recognizeInterimPassportImage(Tesseract, mrzImage, scanner, 0.08, 0.62);
        const mrzData = extractInterimPassportMrz(mrzText);
        if (!mrzData) {
            throw new Error('MRZ non detectee. Recadrez la photo sur les deux lignes en bas du passeport.');
        }

        setInterimPassportStatus(scanner, 'Lecture des details...', 'primary');
        let visualDetails = {};
        try {
            const fullText = await recognizeInterimPassportImage(Tesseract, file, scanner, 0.7, 0.28);
            visualDetails = extractInterimPassportVisualDetails(fullText, mrzData);
        } catch (error) {
            visualDetails = {};
        }

        const updated = applyInterimPassportData(form, {...mrzData, ...visualDetails});
        setInterimPassportProgress(scanner, 1);
        setInterimPassportStatus(scanner, `${updated} champ${updated > 1 ? 's' : ''} rempli${updated > 1 ? 's' : ''}. Verifiez avant d'enregistrer.`, 'success');
        if (!visualDetails.birthPlace || !visualDetails.passportIssuedAt) {
            setInterimPassportWarning(scanner, 'Le lieu de naissance ou la date de delivrance peuvent rester a completer manuellement.');
        }
    } catch (error) {
        setInterimPassportProgress(scanner, 1);
        setInterimPassportStatus(scanner, 'Scan non exploitable.', 'danger');
        setInterimPassportWarning(scanner, error.message || 'Photo illisible. Reessayez avec une image nette.');
    } finally {
        button?.removeAttribute('disabled');
        input.value = '';
        window.setTimeout(() => setInterimPassportProgress(scanner, 0), 450);
    }
}
...
function initializeInterimWorkerForms(root = document) {
    root.querySelectorAll('[data-interim-worker-form]').forEach((form) => {
        ...
        const passportInput = form.querySelector('[data-interim-passport-input]');
        form.querySelector('[data-interim-passport-button]')?.addEventListener('click', () => passportInput?.click());
        passportInput?.addEventListener('change', (event) => {
            handleInterimPassportScan(event.currentTarget).catch((error) => showAlert(error.message, 'danger'));
        });
...
function coutFormatCompact(value) {
...
function calculateCoutRevient(form) {
    ...
    const nombreCaissesNettoyageAuto = poidsProduction > 0 ? poidsProduction / 10 : 0;
    const coutNettoyageKgAuto = poidsProduction > 0 ? (poidsProduction / 30) * 25 : 0;
    const coutMiseEnCaisseKgAuto = poidsFini > 0 ? poidsFini * 2 : 0;
    const coutMainOeuvreKgAuto = coutNettoyageKgAuto + coutMiseEnCaisseKgAuto;
    ...
    if (mode === 'heure') {
        ...
        coutMainOeuvre = coutMainOeuvreKgAuto;
    }
    ...
    return {
        ...
        poidsProductionKgAuto: poidsProduction,
        poidsFiniKgAuto: poidsFini,
        nombreCaissesNettoyageAuto,
        coutNettoyageKgAuto,
        coutMiseEnCaisseKgAuto,
        coutMainOeuvreKgAuto,
        coutMainOeuvre,
        ...
        productionNorms: getProductionNorms(poidsNormeReference),
...
function getProductionNormText(referencePoids, minPercent, maxPercent) {
    if (referencePoids <= 0) {
        return 'Norme -- kg';
    }

    const minKg = (referencePoids * minPercent) / 100;
    const maxKg = (referencePoids * maxPercent) / 100;

    return `Norme ${coutFormatCompact(minKg)}-${coutFormatCompact(maxKg)} kg`;
}
...
function syncCoutRevientForm(form) {
    ...
    form.querySelectorAll('[data-cout-production-norm]').forEach((normBadge) => {
        const key = normBadge.dataset.coutProductionNorm;
        normBadge.textContent = result.productionNorms[key] || 'Norme -- kg';
    });
...
function dailyCostValue(form, name) {
    return coutValue(form, name);
}

function dailyCostNorm(raw, minPercent, maxPercent) {
    if (raw <= 0) return 'Norme -- kg';

    return `Norme ${coutFormatCompact((raw * minPercent) / 100)}-${coutFormatCompact((raw * maxPercent) / 100)} kg`;
}

function dailyCostDiagnostic(raw, finished, waste, loss, totalOutput, gap, yieldPercent) {
    if (raw <= 0 || totalOutput <= 0) {
        return {
            className: 'alert-secondary',
            message: 'Renseignez la quantite sortie, le produit fini, les dechets et les pertes pour analyser la journee.',
        };
    }

    const wasteRate = (waste / raw) * 100;
    const lossRate = (loss / raw) * 100;

    if (Math.abs(gap) > 0.1) {
        return {
            className: 'alert-warning',
            message: `A verifier : PF + dechets + pertes = ${coutFormat(totalOutput)} kg, ecart ${coutFormat(gap)} kg avec la quantite sortie.`,
        };
    }

    if (yieldPercent < 35 || yieldPercent > 60 || lossRate > 12) {
        return {
            className: 'alert-danger',
            message: `Production a surveiller : PF ${coutFormat(yieldPercent)}%, dechets ${coutFormat(wasteRate)}%, pertes ${coutFormat(lossRate)}%.`,
        };
    }

    if (yieldPercent >= 35 && yieldPercent <= 45 && lossRate <= 8) {
        return {
            className: 'alert-success',
            message: `Production normale : rendement ${coutFormat(yieldPercent)}%, dechets ${coutFormat(wasteRate)}%, pertes ${coutFormat(lossRate)}%.`,
        };
    }

    return {
        className: 'alert-warning',
        message: `Production correcte mais hors zone cible anchois filet : PF ${coutFormat(yieldPercent)}%, dechets ${coutFormat(wasteRate)}%, pertes ${coutFormat(lossRate)}%.`,
    };
}

function calculateDailyProductionCost(form) {
    const raw = dailyCostValue(form, 'rawQuantityKg');
    const finished = dailyCostValue(form, 'finishedProductKg');
    const waste = dailyCostValue(form, 'wasteKg');
    const loss = dailyCostValue(form, 'lossKg');
    const totalOutputKg = finished + waste + loss;
    const weightGapKg = totalOutputKg - raw;
    ...
    const laborTotal = hourlyLaborTotal + taskLaborTotal + fixedSalaryDailyTotal;

    const cartonsTotal = dailyCostValue(form, 'cartonCount') * dailyCostValue(form, 'cartonUnitCost');
    const sachetsTotal = dailyCostValue(form, 'sachetCount') * dailyCostValue(form, 'sachetUnitCost');
    const packagingTotal = cartonsTotal + sachetsTotal
        + dailyCostValue(form, 'labelCost')
        + dailyCostValue(form, 'plasticFilmCost')
        + dailyCostValue(form, 'otherPackagingCost');

    const configuredChargesTotal = configuredCoutChargesTotal(form);
    const chargesTotal = configuredChargesTotal + dailyCostValue(form, 'manualChargesAdjustment');
    const totalCost = laborTotal + packagingTotal + chargesTotal;
    const costPerInputKg = raw > 0 ? totalCost / raw : 0;
    const costPerFinishedKg = finished > 0 ? totalCost / finished : 0;
    const alerts = [];

    if (raw > 0 && Math.abs(weightGapKg) > 0.1) alerts.push('Attention : le total PF + dechets + pertes ne correspond pas a la quantite sortie.');
    if (raw > 0 && finished <= 0) alerts.push('Le cout par kg PF ne peut pas etre calcule sans produit fini.');
    if (yieldPercent > 100) alerts.push('Rendement impossible.');
    if (yieldPercent > 0 && yieldPercent < 35) alerts.push('Rendement faible pour un filet anchois standard.');
    ...
    return {
        ...
        costPerFinishedKg,
        norms: {
            finished: dailyCostNorm(raw, 35, 45),
            waste: dailyCostNorm(raw, 50, 60),
            loss: dailyCostNorm(raw, 3, 8),
        },
        diagnostic: dailyCostDiagnostic(raw, finished, waste, loss, totalOutputKg, weightGapKg, yieldPercent),
        alerts,
    };
}

function syncDailyProductionCostForm(form) {
    form.querySelectorAll('input[type="number"]').forEach((input) => {
        if (Number(input.value) < 0) input.value = '0';
    });
    ...
    form.querySelectorAll('[data-daily-cost-norm]').forEach((badge) => {
        badge.textContent = result.norms[badge.dataset.dailyCostNorm] || 'Norme -- kg';
    });

    const diagnostic = form.querySelector('[data-daily-cost-diagnostic]');
    if (diagnostic) {
        diagnostic.className = `alert ${result.diagnostic.className} small mb-0 mt-3`;
        diagnostic.textContent = result.diagnostic.message;
    }

    const alerts = form.querySelector('[data-daily-cost-alerts]');
    if (alerts) {
        alerts.innerHTML = result.alerts.map((alert) => `<div class="alert alert-warning small mb-0">${escapeHtml(alert)}</div>`).join('');
    }
}

function addDailyCostChargeLine(form) {
    const template = form.querySelector('[data-cout-charge-template]');
    const container = form.querySelector('[data-cout-charge-lines]');
    if (!template || !container) return;
    ...
    container.appendChild(row);
    refreshCoutChargeEmptyState(form);
    syncDailyProductionCostForm(form);
    row.querySelector('[data-cout-charge-select]')?.focus();
}

function initializeDailyProductionCostForms(root = document) {
    root.querySelectorAll('[data-daily-production-cost-form]').forEach((form) => {
        syncDailyProductionCostForm(form);
        if (form.dataset.dailyCostInitialized === 'true') return;
        form.dataset.dailyCostInitialized = 'true';
        refreshCoutChargeEmptyState(form);
        form.addEventListener('input', (event) => {
            if (event.target.matches('input, textarea')) syncDailyProductionCostForm(form);
        });
        form.addEventListener('change', (event) => {
            if (event.target.matches('select, input')) syncDailyProductionCostForm(form);
        });
        form.addEventListener('click', (event) => {
            const addButton = event.target.closest('[data-daily-cost-charge-add]');
            if (addButton) {
                addDailyCostChargeLine(form);
                return;
            }
        ...
        form.addEventListener('submit', () => syncDailyProductionCostForm(form));
    });
}
...
function initializePageBehaviors() {
    ...
    initializeDailyProductionCostForms();
...
document.addEventListener('click', async (event) => {
    ...
    if (remoteButton) {
        ...
        try {
            ...
            initializeDailyProductionCostForms(content);
