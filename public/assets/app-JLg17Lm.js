...
function calculateCoutRevient(form) {
    ...
    const coutMatierePremiere = (poidsBrut * coutValue(form, 'prixAchatKg'))
        + coutValue(form, 'fraisTransportAchat')
        + coutValue(form, 'autresFraisAchat');

    const mode = coutField(form, 'modeCalculMainOeuvre')?.value || 'montant_direct';
    let coutMainOeuvre = coutValue(form, 'coutMainOeuvreDirect');
    if (mode === 'heure') {
        coutMainOeuvre = coutValue(form, 'nombreOperatrices') * coutValue(form, 'nombreHeures') * coutValue(form, 'coutHoraireMoyen');
    } else if (mode === 'kg') {
        coutMainOeuvre = coutValue(form, 'prixTacheKg') * coutValue(form, 'kgTraitesMainOeuvre');
    }

    const coutEmballageTotal = (coutValue(form, 'nombreCartons') * coutValue(form, 'prixCarton'))
        + (coutValue(form, 'nombreSachets') * coutValue(form, 'prixSachet'))
    ...
    if (rendementPourcentage > 0 && rendementPourcentage < 40) alerts.push('Rendement faible, verifier pertes et dechets.');
